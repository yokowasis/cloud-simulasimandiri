...
<div id="page">
  <div id="innerpage">
    ...
    <?php 
      global $wpdb;
      $v11sql = "SELECT `nik2`, `server` FROM `{$table_prefix}bsfsm_siswa` WHERE mapel='".$_GET['mapel']."' AND `server` = '$server' GROUP BY `nik2`";
      $rows = $wpdb->get_results($v11sql);
      $kets = $rows;

      global $wpdb;
      $v11sql = "SELECT MAX(`sesi`) AS max FROM `{$table_prefix}bsfsm_siswa` WHERE mapel='".$_GET['mapel']."'";
      $rows = $wpdb->get_results($v11sql);
      $max = $rows[0]->max;
    ?>
    ...
    <table class="maintable">
      ...
      <tr>
        ...
        <th><?php echo ($namaket1) ?></th>
            ...
            <?php
            $v11sql = "SELECT * FROM `{$table_prefix}bsfsm_siswa` WHERE `server` = '".$ket->{'server'}."' AND  mapel='".$_GET['mapel']."' AND `sesi` = '".$i."' ORDER BY `nik2`, `nik`, `kode`";
            $rows = $wpdb->get_results($v11sql);
